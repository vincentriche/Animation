...
void ObjetSimuleMSS::initMeshObjet()
{
	...
	m_ObjetSimule.color(Color(0.94, 0.09, 0.07));
	...
	for (int i = 0; i < _VISize; i++)
		m_ObjetSimule.normal(_vectNormals[_VIndices[i]].x, _vectNormals[_VIndices[i]].y, _vectNormals[_VIndices[i]].z);
...
void ObjetSimuleMSS::updateVertex()
{
	...
	for (int i = 0; i < _VISize; i++)
		m_ObjetSimule.normal(i, _vectNormals[_VIndices[i]].x, _vectNormals[_VIndices[i]].y, _vectNormals[_VIndices[i]].z);
}
...
void ObjetSimuleMSS::Simulation(Vector gravite, float viscosite, int Tps)
{
	...
	if (_Integration == "explicite")
		...
		_SolveurImpl->Solve(viscosite, _Nb_Sommets, Tps, F, A, V, P, M, gravite, _SytemeMasseRessort);
